...
## Overview
Infomaniak Authenticator — TOTP-based two-factor auth app. Kotlin + Jetpack Compose, Hilt DI, Room (via `multiplatform-authenticator/`), and app-lock / device credentials via Infomaniak Core (`com.infomaniak.core.applock`). Also exports an iOS Swift Package / XCFramework for cross-platform logic (`multiplatform-authenticator/`). Two flavors: `standard` and `fdroid`.

## One-Time Environment Setup
```bash
git submodule update --init --recursive   # Core and multiplatform-authenticator submodules
...
multiplatform-authenticator/  # Git submodule — KMP module (TOTP logic shared with iOS)
...
- After modifying the public API of `multiplatform-authenticator/`, verify the iOS XCFramework build still compiles (`./buildXCFramework`).
