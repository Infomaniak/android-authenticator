...
> Multiplatform library norms, see `multiplatform-authenticator/AGENTS.md`. For Core library norms, see `Core/AGENTS.md`.

## Repository Structure

This is a composite Gradle build that ships the Infomaniak Authenticator Android app and includes a Kotlin Multiplatform
library (Git submodule, own repo) shared with the iOS/macOS Authenticator (distributed as an XCFramework + Swift Package).
...
├── multiplatform-authenticator/  # Git submodule - Kotlin Multiplatform shared library (see multiplatform-authenticator/AGENTS.md), included as a composite build
│   └── AuthenticatorCore/        # Gradle module holding the shared business logic
...
| Multiplatform Lib | `multiplatform-authenticator/` | `multiplatform-authenticator/AGENTS.md` | Shared Kotlin Multiplatform code (Android + iOS/macOS) - OTP, API, migration (Git submodule) |
| Root              | `./`                           | `AGENTS.md`                             | This file - composite build overview                                                         |

## Composite Build Explained

- **`Core` and `multiplatform-authenticator` are Git submodules**: Changes in `Core/` are tracked separately and shared with
  other Infomaniak apps; changes in `multiplatform-authenticator/` are tracked in its own repo and shared with the iOS/macOS
  Authenticator. Always run `git submodule update --init --recursive` after cloning, otherwise the Gradle build will fail to
  resolve `com.infomaniak.core.composite` and the `multiplatform-authenticator` sources.
...
- **`multiplatform-authenticator` is included as a composite build** via `includeBuild("multiplatform-authenticator")` in
  `settings.gradle.kts`, with a dependency substitution mapping the `com.infomaniak.multiplaform-authenticator:multiplatform-authenticator`
  module to the `:AuthenticatorCore` project.
- **Version catalogs**:
    ...
    - `kmpAuthenticator` - catalog provided by the multiplatform submodule
      (`multiplatform-authenticator/gradle/kmpAuthenticator.versions.toml`), registered in `settings.gradle.kts`
...
```bash
# Initialize Core and multiplatform-authenticator submodules (required for any Gradle command)
...
./gradlew :multiplatform-authenticator:AuthenticatorCore:assemble

# Build the iOS/macOS XCFramework (CoreAuthenticator.xcframework)
./buildXCFramework

# Common multiplatform tests
./gradlew :multiplatform-authenticator:AuthenticatorCore:allTests
